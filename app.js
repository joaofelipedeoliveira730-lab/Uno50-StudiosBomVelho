...
const MAP_PERSONALITY={
  saloon:{label:'Saloon clássico',music:'saloon',decor:'  '},medieval:{label:'Taverna medieval',music:'medieval',decor:'  '},pirate:{label:'Navio pirata',music:'pirate',decor:'  '},modern:{label:'Casa moderna',music:'modern',decor:'  '},classroom:{label:'Sala de aula',music:'modern',decor:'  '},neon:{label:'Cidade neon',music:'modern',decor:'  '},forest:{label:'Floresta',music:'forest',decor:'  '},desert:{label:'Deserto',music:'saloon',decor:' '},ice:{label:'Montanha congelada',music:'modern',decor:' '},space:{label:'Estação espacial',music:'modern',decor:' '},math:{label:'Dimensão Matematixa',music:'modern',decor:'∞  '},ceo:{label:'Dimensão CEO',music:'modern',decor:'  '},
  'velho-bar':{label:'Bar do Velho',music:'saloon',decor:'    '},
...
function authErrorMessage(err, fallback){
  ...
  if(typeof raw === 'string' && raw.trim() && !/^\[object (PointerEvent|MouseEvent|Object)\]$/i.test(raw.trim())) return raw.trim();
  if(err?.data?.message && typeof err.data.message === 'string') return err.data.message;
  return fallback;
}
async function login(e){
  e?.preventDefault?.();
  const form=e?.currentTarget || $('#formLogin');
  if(!form)return;
  ...
  try{
    ...
    setMsg('#loginMessage','Entrando...');
    if(!username||!password) throw new Error('Informe usuário e senha.');
    const d=await post('/login',{username,password});
    state.token=d.token||null;
    if(state.token)localStorage.setItem('uv_token',state.token);
    state.user=d.user;state.profile=normalizeProfile(d.profile);
    await enterApp(false);
    setMsg('#loginMessage','Login realizado!','success');Sound.ok();
  }catch(err){
    setMsg('#loginMessage',authErrorMessage(err,'Não foi possível entrar. Verifique os dados e tente novamente.'),'error');
    Sound.bad();
  }finally{if(btn)btn.disabled=false;}
}
async function register(e){
  e?.preventDefault?.();
  const form=e?.currentTarget || $('#formRegister');
  if(!form)return;
  const fd=new FormData(form);
  const username=String(fd.get('regUsername')||'').trim();
  const password=String(fd.get('regPassword')||'');
  const btn=e?.submitter || form.querySelector('button[type=submit]');
  try{
    if(btn)btn.disabled=true;
    setMsg('#registerMessage','Criando conta...');
    if(!/^[A-Za-z0-9_]{3,24}$/.test(username)) throw new Error('Usuário deve ter 3-24 caracteres, usando apenas letras, números ou _.');
    if(password.length<6) throw new Error('A senha precisa ter pelo menos 6 caracteres.');
    const d=await post('/register',{username,password});
    state.token=d.token||null;
    if(state.token)localStorage.setItem('uv_token',state.token);
    state.user=d.user;state.profile=normalizeProfile(d.profile);
    await enterApp(true);Sound.ok();
  }catch(err){
    setMsg('#registerMessage',authErrorMessage(err,'Não foi possível criar a conta. Tente novamente.'),'error');
    Sound.bad();
  }finally{if(btn)btn.disabled=false;}
}
...
function renderDrawingState(g){
  ...
  const mine=String(g.drawerId)===String(state.user?.id);const title=$('#drawTurnLabel');if(title)title.textContent=mine?' SUA VEZ DE DESENHAR':' '+(g.drawerName||'Jogador')+' está desenhando';
...
function handleGameAction(action={}){
  ...
  else if(action.type==='uno-penalty'){Sound.bad();toast(` ${escapeHtml(action.username||'Jogador')} recebeu ${action.count||2} cartas.`,'error',1800);}
}
...
function renderModeGameState(g){
  ...
  const labels={truco:[' TRUCO','Truco do Velho'],checkers:[' DAMAS','Damas de Botecão'],chess:[' XADREZ','Xadrez do Bar']};
...
async function toggleMaintenance(){
  ...
  try{
    if(active){
      ...
      if(btn)btn.textContent=' ATIVAR MODO MANUTENÇÃO';
      ...
      if(btn)btn.textContent='▶ LIBERAR O JOGO';
...
function applyMaintenance(paused,message){
  ...
  if(paused){
    ...
    if($('#globalPauseBanner'))$('#globalPauseBanner').textContent=' JOGO EM MANUTENÇÃO';
...
async function openCEOPanel(){
  ...
  try{
    ...
    const btn=$('#btnMaintenance');
    if(btn)btn.textContent=state.maintenance?'▶ LIBERAR O JOGO':' ATIVAR MODO MANUTENÇÃO';
...
async function openShop(mode='official'){
  ...
  }catch(e){grid.innerHTML=`<div class="empty-state glass"> ${escapeHtml(e.message)}</div>`;}
}
...
async function openInventory(mode='items'){
  ...
  }catch(e){if(el)el.innerHTML=`<div class="empty-state"> ${escapeHtml(e.message)}</div>`;}
}
  ...
  try{
    ...
    grid.innerHTML=(d.levels||[]).map(r=>{
      ...
      return `<article class="pass-card glass ${unlocked?'unlocked':'locked'} ${done?'claimed':''}">
        <div class="pass-level">NÍVEL ${r.level}</div><div class="pass-reward-icon">${r.itemId?'':r.title?'':''}</div>
...
function renderEquippedList(owned){
  ...
  const slots=[['top',''],['hair',''],['oponentetom',''],['shoes',''],['accessory','']];
...
/* =====================================================================
   MATEMATIXA 8.0 — tactile presentation layer
   No gameplay state is changed here. It only adds visual feedback.
   ===================================================================== */
(function premiumPresentation(){
  ...
  const ripple=(el,e)=>{
    ...
    const rect=el.getBoundingClientRect(); const size=Math.max(rect.width,rect.height)*.8;
    r.style.width=r.style.height=size+'px'; r.style.left=(e.clientX-rect.left-size/2)+'px'; r.style.top=(e.clientY-rect.top-size/2)+'px';
    el.style.position=el.style.position==='static'?'relative':el.style.position; el.appendChild(r); setTimeout(()=>r.remove(),600);
  };
  function bind(){
    document.querySelectorAll('button').forEach(btn=>{
      if(btn.dataset.premiumBound) return; btn.dataset.premiumBound='1';
      btn.addEventListener('pointerdown',e=>{ripple(btn,e); if(!reduced()){btn.classList.remove('click-lift'); void btn.offsetWidth; btn.classList.add('click-lift');}}, {passive:true});
    });
    document.querySelectorAll('.hand-card,.uno-card').forEach(card=>{
      if(card.dataset.premiumCard) return; card.dataset.premiumCard='1';
      card.addEventListener('click',()=>{if(!reduced()){card.classList.remove('card-play-pop');void card.offsetWidth;card.classList.add('card-play-pop');}});
    });
  }
  function particles(){
    const targets=document.querySelectorAll('.lobby-v2,.first-person-arena');
    targets.forEach(target=>{
      if(target.dataset.particles) return; target.dataset.particles='1';
      const frag=document.createDocumentFragment();
      for(let i=0;i<18;i++){
        const p=document.createElement('i'); p.className='premium-particle'+(i%3===0?' purple':'');
        p.style.left=(8+Math.random()*84)+'%'; p.style.bottom=(5+Math.random()*25)+'%';
        p.style.setProperty('--dx',((Math.random()-.5)*120)+'px'); p.style.setProperty('--dur',(5+Math.random()*7)+'s'); p.style.animationDelay=(-Math.random()*8)+'s'; frag.appendChild(p);
      }
      target.appendChild(frag);
    });
  }
  const observer=new MutationObserver(()=>{bind();particles();});
  const boot=()=>{bind();particles();observer.observe(document.body,{childList:true,subtree:true});};
